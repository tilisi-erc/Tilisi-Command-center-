...
<script>
let ventures = JSON.parse(localStorage.getItem('elshaddai_v3') || 'null') || [
...
function render(){
  ...
  ventures.forEach((v,i)=>{
    let diff=v.val-v.cap, perc=(v.val/v.cap-1)*100;
    ...
    let width=Math.min(100, (v.val/v.cap)*70);
    h+=`<div class="row">
      ...
      <div><div class="s-lbl">CAPITAL</div><div style="font-weight:800">KES ${v.cap.toLocaleString()}</div><div class="s-lbl" style="margin-top:4px">VALUE</div><div style="font-weight:800">KES ${v.val.toLocaleString()}</div></div>
      <div><div class="s-lbl">PnL / GROWTH</div><div style="color:${diff>=0?'#22c55e':'#ef4444'};font-weight:800">${diff>=0?'+':''}${diff.toLocaleString()} (${perc.toFixed(1)}%)</div><div class="s-lbl" style="margin-top:4px">INCOME</div><div style="font-weight:800;color:#facc15">KES ${v.inc.toLocaleString()}/mo</div></div>
